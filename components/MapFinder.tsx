...
const MapFinder: React.FC<Props> = ({ lang }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-white/95 backdrop-blur rounded-2xl shadow-2xl overflow-hidden p-6 animate-slide-up min-h-[500px]">
      <div className="text-center mb-8">
        <div className="mx-auto w-12 h-12 bg-green-100 rounded-full flex items-center justify-center text-green-600 mb-3">
          <MapPinIcon />
        </div>
        <h2 className="text-2xl font-bold text-gray-800">{t.findTitle}</h2>
        <p className="text-gray-500">{t.findDesc}</p>
      </div>
      ...
      <form onSubmit={handleSearch} className="mb-8">
        <div className="flex gap-2">
          <input 
            ...
            className="flex-1 px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none text-gray-800"
          />
          <button 
            type="submit" 
            disabled={loading}
          ...
          {result.places.length > 0 && (
            <div className="grid gap-3">
              {result.places.map((place, idx) => (
                ...
                >
                  <span className="font-medium text-gray-800 group-hover:text-green-800">{place.title}</span>
                  <MapPinIcon />
                </a>
              ))}
            </div>
